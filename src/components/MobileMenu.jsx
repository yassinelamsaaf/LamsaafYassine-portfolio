...
export const MobileMenu = ({ menuOpen, setMenuOpen, theme, onToggleTheme }) => {
  ...
  return (
    ...
    >
      ...
      <aside
        className={`absolute right-0 top-0 flex h-full w-[84vw] max-w-sm flex-col border-l border-blue-400/15 bg-blue-950/90 px-5 py-6 backdrop-blur-xl transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
        ...
        <nav className="mt-8 flex flex-col gap-2">
          {navLinks.map((link) => (
            <a
              ...
              href={link.href}
              onClick={() => setMenuOpen(false)}
              className="rounded-2xl border border-blue-400/15 bg-black/20 px-4 py-4 text-lg font-medium text-white transition hover:border-blue-300/30 hover:bg-black/30"
            >
              {link.label}
            </a>
          ))}
